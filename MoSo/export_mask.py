...
from dataset_utils import REPO_ROOT, build_eval_train_dataset, get_dataset_targets
...
def resolve_score_path(path: str | None, score_root: str | None) -> Path:
    if score_root:
        return Path(score_root).expanduser().resolve() / 'moso_score.pth'
    if not path:
        raise ValueError('Either --path or --score_root must be provided to locate moso_score.pth.')
    return Path(path).expanduser().resolve() / 'score' / 'moso_score.pth'
...
def export_mask(dataset: str, seed: int, keep_ratio: int, data_root: str, path: str | None, score_root: str | None, mask_root: str | None, random_mode: bool = False) -> Path:
    ...
    mask_base = Path(mask_root).expanduser().resolve() if mask_root else (REPO_ROOT / 'mask').resolve()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mask_root', default=None, type=str, help='Optional mask root, defaults to repo_root/mask.')
